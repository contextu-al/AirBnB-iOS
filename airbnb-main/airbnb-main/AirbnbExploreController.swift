...
//  airbnb-main
//
//  Created by Yonas Stephen on 2017/3/11.
...
class AirbnbExploreController: UIViewController {
    ...
    var experienceController: AirbnbExploreExperienceController = {
        let controller = AirbnbExploreExperienceController()
        controller.title = "Experience"
        return controller
    }()
    
    var placeController: AirbnbExplorePlacesController = {
        let controller = AirbnbExplorePlacesController()
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if isFirstAppear {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now()+2, execute: {
                self.setContent(toViewControllerAtIndex: 0)
            })
        }else{
            currentPageController?.viewDidAppear(animated)
        }
    }
