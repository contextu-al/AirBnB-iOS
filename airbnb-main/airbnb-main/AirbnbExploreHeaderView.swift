...
class AirbnbExploreHeaderView: UIView, AirbnbLocationControllerDelegate {
    
    public var city: LocationItem?
    ...
    lazy var destinationFilter: UIButton = {
        ...
        btn.titleEdgeInsets = UIEdgeInsets(top: 0, left: 10, bottom: 0, right: 0)
        btn.titleLabel?.lineBreakMode = .byTruncatingTail
        btn.addTarget(self, action: #selector(AirbnbExploreHeaderView.showLocation), for: .touchUpInside)
    ...
    @objc func showLocation(){
        let locationVC = AirbnbLocationController()
        locationVC.delegate = self
        locationVC.selectedCity = self.city
        ...
        AppFieldEditGuide.sharedInstance.hasInteractedWithFilter = true
    }
    
    func locationController(_ locationController: AirbnbLocationController, didSaveCity city: LocationItem?) {
        self.city = city
        if let city = city {
            destinationFilter.setTitle( city.cityAndState, for: .normal)
        }else{
            destinationFilter.setTitle( "Anywhere", for: .normal)
        }
    }
...
//        }
    ...
    public func updateHeader(newHeight: CGFloat, offset: CGFloat) {
        ...
        self.delegate?.tabBarController?.tabBar.backgroundColor = .white
    }
